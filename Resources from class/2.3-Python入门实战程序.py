...
并且，python利用代码间的缩进表示隶属关系。

如果是初学，可以将每个函数单独截取阅读并运行查看。
当然咯，代码不能光读不写，一定要动手去编。如果不动手编，可能看完运行完你依旧还是不是很懂.
"""
...
def study_loop_select():
    ...
    while i <= num:  # 108、while expression:当expression为真的时候进行循环
        ...
        if i > 20:
            ...
            break  # 111、break语句，运用在循环中，直接退出循环

        time.sleep(0.5)  # 112、time库为python中的时间库，time.sleep(second)可以使程序暂停运行second秒

    else:  # 113、while循环后接一个else语句，当执行完所有循环后执行一次
        print('while结束了')
